        ...
        <form wire:submit.prevent="save()" id="addform">
            ...
            <div class="row">
                ...
                <div class="col-md-6 mb-3" wire:ignore>
                    <select name="day" id="day" multiple="multiple" class="form-select position-relative z-10 select2 @error('detail.day') is-invalid @enderror">  
                        @foreach ($days as $key => $value )
                            @if(in_array($value['day'], $detail['day']))
                                <option selected value="{{ $value['day'] }}"x >{{$value['day_full'] }}</option>
                            @else
                                <option value="{{ $value['day'] }}"x >{{$value['day_full'] }}</option>
                            @endif
                        @endforeach
                    </select>
                </div>
